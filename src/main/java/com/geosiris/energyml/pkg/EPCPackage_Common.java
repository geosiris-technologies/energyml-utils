...
public class EPCPackage_Common extends EPCPackage {
	private final static String staticPkgPath = "energyml.common";
	...
	public EPCPackage_Common(String xsdMappingFilePath) {
		super("eml", staticPkgPath, xsdMappingFilePath);
		logger.info("EPCPackage_Common initialized " + pkgClasses.keySet());
	}

	@Override
	public Object createInstance(String className, String version) {
		Object result = super.createInstance(className, version);
		if(result != null && result.getClass().getSimpleName().toLowerCase().compareTo("parametertemplate")==0) {
			try {
				ObjectController.editObjectAttribute(result, "MaxOccurs", "1");
				ObjectController.editObjectAttribute(result, "MinOccurs", "0");
				ObjectController.editObjectAttribute(result, "IsInput", "true");
